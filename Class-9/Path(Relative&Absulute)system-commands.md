...
# 📒 Notes: Absolute Path vs Relative Path in Linux

### 🔹 Absolute Path

* **Definition:**
  Absolute path always **starts from the root directory (`/`)**.
  It shows the complete location of a file or directory.

* **Example:**

  ```bash
  /home/jay/Documents/file.txt
  /var/log/syslog
  /etc/passwd
  ```

  No matter where you are, this path will always point to the same file/directory.
...
* **Definition:**
  Relative path means the **path calculated from the current working directory (CWD)**.
...
* **Example:**
  ...
  ```bash
  cd Documents
  cat ./file.txt        # file inside current directory
  cd ..
  cd ../Downloads       # go to Downloads from parent directory
  ```
...
| -------------- | ----------------------------------- | ------------------------------------- |
| Starting Point | Starts from root `/`                | Starts from current working directory |
| Example        | `/home/jay/Documents/file.txt`      | `Documents/file.txt`                  |
| Flexibility    | Always fixed and correct            | Depends on current location           |
| Use Case       | Mostly used in scripts/config files | Used for quick navigation             |
